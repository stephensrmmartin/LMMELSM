...
transformed parameters {
  ...
  // Empirical innovation
  matrix[N, F] var_innovation = rep_matrix(0, N, F);
  ...
  // ARMA Scale
  var_innovation = log(sqrt(square(y - mu_hat)));
  for(n in 2:N) {
    // AR
    for(lag in 1:min(AR_Q, n - 1)) {
      //Empirical innovation
      /* logsd_hat[n] += logsd_hat[n - lag] .* ar_scale[lag]; */
      logsd_hat[n] += (logsd_hat[n - lag] + var_innovation[n - lag]) .* ar_scale[lag];
    }
    // MA
  ...
  // Empirical innovation
    var_innovation[n] -= logsd_hat[n];
  // Empirical innovation (Include or not?) (To model w/ sigma or not?)
    /* logsd_hat[n] += var_innovation[n]; */
  }
}
...
model {
  ...
  for(n in 1:N) {
    ...
    var_innovation[n] ~ normal(0, var_innovation_sd);
  }
}
...
generated quantities {
  ...
  for(a in (max_lag + 1):(max_lag + ahead)) {
    ...
    for(lag in 1:min(AR_Q, a - 1)) {
      ...
      // Empirical
      logsd_hat_comb[a] += (logsd_hat_comb[a - lag] + var_innovation_comb[a - lag]) .* ar_scale[lag];
    }
    ...
    // Empirical innovation
    var_innovation_comb[a] = log(sqrt(square(y_comb[a] - mu_hat_comb[a]))) - logsd_hat_comb[a];
  }
}
